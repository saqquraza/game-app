...
const CheckOut = ({ cart, setCart, handleChange }) => {
  ...
  const handlePrice = () => {
    ...
    cart.map(
      (item) =>
        (ans +=
          item.distance * 2 > 30 ? ((item.distance * 2 - 30) * 50 + 1500)*item.amount : 1500*item.amount )
    );
    setPrice(ans);
  };
 ...
 {cart.map((item) => (
     ...
     <div onClick={()=>setShow(true)}  className="payment">Go for payment</div>
      </>
    }
    </article>
  );
